...
export async function requireFirebaseUser(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  if (scheme !== 'Bearer' || !token) return res.status(401).json({ error: 'Missing Firebase ID token' });
  try {
...
export async function requireFirebaseAdmin(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  await requireFirebaseUser(req, res, () => {
    const user = req.firebaseUser;
    if (!user) return res.status(401).json({ error: 'Unauthorized' });
    const email = typeof user.email === 'string' ? user.email.toLowerCase() : '';
    if (user.admin !== true && !configuredAdminEmails().has(email)) {
